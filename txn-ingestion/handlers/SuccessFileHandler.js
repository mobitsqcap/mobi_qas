...
class SuccessFileHandler {
  ...
  async handle(file, context) {
    const { processingPath, completedPath, case: successCase, validRecords } = context;
    if (!completedPath) throw new Error(`Completed path missing for file ${file.name}`);
    if (successCase === 'FULL') { await this.sftpService.moveFile(processingPath, completedPath); file.path = completedPath; }
    else if (successCase === 'PARTIAL') { const buffer = this._buildValidRecordsCsv(validRecords); await this.sftpService.uploadFile(completedPath, buffer); }
    else throw new Error(`SuccessFileHandler: unknown case '${successCase}'`);
  }
  ...
  _buildValidRecordsCsv(validRecords) {
    const headers = Constants.TRANSACTION_CSV_COLUMNS;
    return Buffer.from(CsvUtil.serialize(headers, validRecords.map((r) => this._toSourceCsvRow(r))), 'utf-8');
  }
}
