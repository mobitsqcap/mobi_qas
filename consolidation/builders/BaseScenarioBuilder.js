...
class BaseScenarioBuilder {
  ...
  async buildDocuments() { throw new Error('buildDocuments must be implemented by scenario builder'); }
  ...
  createHeader({ consolRefId, sapRefDocument, groupRows, totals, options={} }) {
    ...
    return {
      ...
      POSTING_STATUS:     this.scenario.postingStatus,   // '02'
      POST_DATE:          null,
      HTTP_STATUS:        null,
      ERROR_CODE:         null,
      ERROR_DETAIL:       null,
  ...
  createLineItem({
    ...
    const line = {
      ...
      HOST_MDR_AMOUNT:null, HOST_FEE_PAYABLE:null, MDR_REVENUE:null,
      AR_PAYIN:null, AP_PAYIN:null, AP_PAYOUT:null, TRANSACTION_AMOUNT:null,
      ...
      DOCUMENT_TYPE: this.scenario.documentType,
      POSTING_STATUS:this.scenario.postingStatus,
      POST_DATE:null, HTTP_STATUS:null, ERROR_CODE:null, ERROR_DETAIL:null,
      RETRY_COUNT:0,
      CREATED_BY:user, CREATED_TIMESTAMP:now, CHANGED_BY:null, CHANGED_TIMESTAMP:null
    };
  ...
  totalsFromLineItems(lineItems) {
    ...
    for (const line of lineItems || []) {
      ...
      if (line.DEBIT_CREDIT_INDICATOR === Constants.DEBIT_CREDIT.DEBIT)       debitCents  += AmountUtil.toCents(amt);
      else if (line.DEBIT_CREDIT_INDICATOR === Constants.DEBIT_CREDIT.CREDIT) creditCents += AmountUtil.toCents(amt);
    }
  ...
  validateBalanced(document) {
    ...
    if (debit !== credit) {
      throw new Error(
        `Consolidation ${document.header.CONSOL_REF_ID} is not balanced: debit ${document.header.TOTAL_DEBIT_AMOUNT}, credit ${document.header.TOTAL_CREDIT_AMOUNT}`
      );
    }
  }
}
