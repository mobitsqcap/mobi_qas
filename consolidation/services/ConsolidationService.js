...
//         await this.auditRepository.insertTransactionErrors({
//           runId, scenarioCode: scenario.code, records: errorRecords.map((e) => e.record),
...
//       // Point 5: zero GL/BP errors on a successful run -> freeze any previous
//       // error report by renaming it to Transactions_YYYYMMDD_HHMMSS.csv.
//       if (!errorRecords.length) {
//         const resolved = await this._resolveConsolidationErrorReport({ postingDate: options.postingDate });
...
//           errorDetail: skipped
//             ? `${successTxnCount} consolidated, ${skipped} recoverable errors`
...
//    */
//   async _resolveConsolidationErrorReport({ postingDate }) {
//     try {
//       return await this.errorReporter.resolveErrorReport({ postingDate });
//     } catch (error) {
