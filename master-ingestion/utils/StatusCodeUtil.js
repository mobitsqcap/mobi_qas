...
duplicateBpInDb: (id) =>
  `BP ID "${id}" already exists in the system. BP IDs must be globally unique irrespective of Company Code or Portal. Please use a different BP ID or update the existing record.`,
 ...
 * Safe to call on every service start - INSERTs are guarded so duplicates are
  ...
  try {
    ...
    for (let i = 0; i < entries.length; i += chunkSize) {
      ...
      try {
        ...
        // Row-by-row fallback if the dialect rejects the bulk insert
